    ...
    if meta['snsSaveChanSubset'] == 'all':
        ...
        for sL in chStrList:
            ...
            if len(currList) > 1:
                ...
                newChans = np.arange(int(currList[0]), int(currList[1]))  # Eliminated "+1" to make sure nChannels = len(APgain)
            else:
                newChans = np.arange(int(currList[0]), int(currList[0]))
...
# of these tools.
#
def makeMemMapRaw(binFullPath, meta, start_t, duration):
    nChan = int(meta['nSavedChans'])
    nFileSamp = int(int(meta['fileSizeBytes'])/(2*nChan))
    # print("nChan: %d, nFileSamp: %d" % (nChan, nFileSamp), 'Sample Rate: ', SampRate(meta))

    start_FileSamp = int(start_t * SampRate(meta)*2)
    end_FileSamp = int(start_t * SampRate(meta)*2 + duration * SampRate(meta))
    # print(start_FileSamp, end_FileSamp)
    rawData = np.memmap(binFullPath, dtype='int16', mode='r',
                        shape=(nChan, end_FileSamp-start_FileSamp), offset=start_FileSamp*nChan, order='F')
    # print(rawData.shape)
    return(rawData)
